...
impl Button {
    ...
    pub fn new(img: Image, pct_x :f32 ,pct_y :f32,pct_w : f32,pct_h: f32) -> Button {
        Button { 
            img: img,
            pct_rect: Rect { x: pct_x,y: pct_y,w: pct_w,h:pct_h },
        }
    }
    ...
    pub fn left_clicked(&self,ctx:&mut Context,mouse_state:&MouseState) -> bool {
        match mouse_state {
            MouseState::Up(button_state) =>
                button_state.which_button == MouseButton::Left && self.pixel_rect(ctx).contains(na::Point2::new(button_state.x,button_state.y)),
            _ => false
        }
    }
    ...
    pub fn right_clicked(&self,ctx:&mut Context,mouse_state:&MouseState) -> bool {
        match mouse_state {
            MouseState::Up(button_state) =>
                button_state.which_button == MouseButton::Right && self.pixel_rect(ctx).contains(na::Point2::new(button_state.x,button_state.y)),
            _ => false
        }
    }
    ...
    pub fn draw(&self,ctx:&mut Context) {
        ...
        let params = DrawParam::new().dest(na::Point2::new(pixel_rect.x,pixel_rect.y)).scale(na::Vector2::new(x_scale,y_scale));
